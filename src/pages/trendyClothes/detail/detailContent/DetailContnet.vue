...
export default {
  ...
  computed:{
    ...mapState('getInfo',['zhaoliudata'])
  },
  created() {
    setTimeout(() => {
      this.shuJu = this.zhaoliudata
      // console.log(this.shuJu)
      for (let i = 0;i < this.zhaoliudata.length;i++){
        this.name.push(this.zhaoliudata[i].name)
      }
      for (let i = 0;i < this.zhaoliudata.length;i++){
        this.description.push(this.zhaoliudata[i].description)
      }
      for (let i = 0;i < this.zhaoliudata.length;i++){
        this.discount.push(this.zhaoliudata[i].discount)
      }
      for (let i = 0;i < this.zhaoliudata.length;i++){
        this.price.push(this.zhaoliudata[i].price)
      }
      for (let i = 0;i < this.zhaoliudata.length;i++){
        this.imgname.push(this.zhaoliudata[i].imgName)
      }
    },2000)
  },
  components:{
