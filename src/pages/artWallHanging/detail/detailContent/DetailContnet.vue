...
export default {
  ...
  computed:{
    ...mapState('getInfo',['biguadata'])
  },
  created() {
    setTimeout(() => {
      this.shuJu = this.biguadata
      // console.log(this.shuJu)
      for (let i = 0;i < this.biguadata.length;i++){
        this.name.push(this.biguadata[i].name)
      }
      for (let i = 0;i < this.biguadata.length;i++){
        this.description.push(this.biguadata[i].description)
      }
      for (let i = 0;i < this.biguadata.length;i++){
        this.discount.push(this.biguadata[i].discount)
      }
      for (let i = 0;i < this.biguadata.length;i++){
        this.price.push(this.biguadata[i].price)
      }
      for (let i = 0;i < this.biguadata.length;i++){
        this.imgname.push(this.biguadata[i].imgName)
      }
    },2000)
  },
  components:{
