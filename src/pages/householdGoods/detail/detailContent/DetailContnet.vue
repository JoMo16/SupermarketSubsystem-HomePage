  ...
  created() {
    setTimeout(() => {
      ...
      for (let i = 0;i < this.homedata.length;i++){
        this.name.push(this.homedata[i].name)
      }
      for (let i = 0;i < this.homedata.length;i++){
        this.description.push(this.homedata[i].description)
      }
      for (let i = 0;i < this.homedata.length;i++){
        this.discount.push(this.homedata[i].discount)
      }
      for (let i = 0;i < this.homedata.length;i++){
        this.price.push(this.homedata[i].price)
      }
      for (let i = 0;i < this.homedata.length;i++){
        this.imgname.push(this.homedata[i].imgName)
      }
    },2000)
  },
  components:{
